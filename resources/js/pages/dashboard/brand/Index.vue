...
import type { BrandIndexProps, BrandItem } from '@product/types';
...
const props = defineProps<BrandIndexProps>();
...
const selectedItem = ref<BrandItem | null>(null);
const selectedUuids = ref<(string | number)[]>([]);

// Pagination computed (transform meta to top-level for TableReusable)
const pagination = computed(() => ({
    current_page: props.brands.meta.current_page,
    last_page: props.brands.meta.last_page,
    per_page: props.brands.meta.per_page,
    total: props.brands.meta.total,
}));
...
const columns: TableColumn<BrandItem>[] = [
    ...
    { key: 'website', label: 'Website', width: '15%' },
...
// Table actions
const tableActions: TableAction<BrandItem>[] = [
    {
        label: 'View',
        icon: Eye,
        onClick: (item) => router.visit(`/dashboard/brands/${item.id}`),
    },
    {
        label: 'Edit',
        icon: Pencil,
        onClick: (item) => router.visit(`/dashboard/brands/${item.id}/edit`),
    },
    {
        label: 'Delete',
        icon: Trash2,
...
const deleteModalTitle = computed(() => {
    ...
    if (count > 0) {
        return `Delete Brand (${count} Products)`;
    }
    return 'Delete Brand';
});
...
const deleteModalDescription = computed(() => {
    ...
    const count = selectedItem.value.products_count ?? 0;
    if (count > 0) {
        return `Warning: "${selectedItem.value.name}" has ${count} product(s) associated with it. Deleting this brand will unassign these products from this brand. This action will move it to trash.`;
    }
...
// Handlers
const handleCreate = () => router.visit('/dashboard/brands/create');

const handleTrash = () => router.visit('/dashboard/brands/trash');
...
const handleDelete = () => {
    ...
    router.delete(`/dashboard/brands/${selectedItem.value.id}`, {
        onSuccess: () => {
...
const openBulkDeleteDialog = () => {
    ...
    router.visit(`/dashboard/brands/bulk-delete?${params.toString()}`);
};

const handleRowClick = (item: BrandItem) => {
    router.visit(`/dashboard/brands/${item.id}`);
};
...
const handleClearFilters = () => {
    ...
    router.get('/dashboard/brands', {}, { preserveState: true, preserveScroll: true });
};

const handleStatusToggle = (item: BrandItem, newStatus: boolean) => {
    router.put(`/dashboard/brands/${item.id}/toggle-status`, {
        is_active: newStatus,
    }, {
        preserveState: true,
        preserveScroll: true,
    ...
    <div class="flex h-full flex-1 flex-col gap-6 p-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-muted-foreground">Manage your product brands</p>
            </div>
            <div class="flex items-center gap-2">
                ...
                <Button variant="outline" as="a" href="/dashboard/brands/export">
        ...
        >
            ...
            <template #cell-name="{ item }">
                <div class="flex items-center gap-3">
                    <Avatar class="h-9 w-9">
                        <AvatarImage v-if="item.logo" :src="item.logo" :alt="item.name" />
            ...
            <template #cell-products_count="{ item }">
                <Badge
                    ...
                    @click.stop="router.visit(`/dashboard/products?brand_id=${item.id}`)"
                >
            ...
            <template #cell-website="{ item }">
                ...
                >
                    <Globe class="h-3 w-3" />
                    <span class="max-w-[120px] truncate">{{ item.website.replace(/^https?:\/\//, '') }}</span>
                </a>
            ...
            <div class="flex items-start gap-3 rounded-lg border border-yellow-500/50 bg-yellow-500/10 p-3 mt-4">
                ...
                <div class="text-sm">
                    <p class="font-medium text-yellow-700 dark:text-yellow-400">
                        This brand has {{ selectedItem?.products_count }} products
                    </p>
                    <p class="text-muted-foreground mt-1">
                        Products will have their brand unassigned but won't be deleted.
                    </p>
                </div>
            </div>
        </template>
