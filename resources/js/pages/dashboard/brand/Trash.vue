...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Trash', href: '/dashboard/brands/trash' },
];

const trashConfig: TrashConfigLocal = {
    entityLabel: 'Brand',
    entityLabelPlural: 'Brands',
    restoreRoute: (uuid: string) => `/dashboard/brands/${uuid}/restore`,
    forceDeleteRoute: (uuid: string) => `/dashboard/brands/${uuid}/force-delete`,
    listRoute: '/dashboard/brands/trash',
};
...
const handlePageChange = (page: number) => {
    router.get('/dashboard/brands/trash', {
        page,
...
const handleBulkRestore = () => {
    router.put('/dashboard/brands/trash/bulk-restore', {
...
const handleBulkForceDelete = () => {
    if (confirm(`Are you sure you want to permanently delete ${selectedUuids.value.length} item(s)? This action cannot be undone.`)) {
        router.delete('/dashboard/brands/trash/bulk-force-delete', {
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        <Head title="Brands Trash" />

        <div class="flex h-full flex-1 flex-col gap-6 p-6">
            <!-- Header -->
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold">Brands Trash</h1>
                    <p class="text-sm text-muted-foreground">
                        Manage deleted brands - restore or permanently delete
                    </p>
                </div>
                <ButtonGroup>
            ...
            <TrashTable
                ...
                :selectable="true"
                select-key="uuid"
                empty-message="No deleted brands found."
                empty-trash-route="/dashboard/brands/trash/empty"
